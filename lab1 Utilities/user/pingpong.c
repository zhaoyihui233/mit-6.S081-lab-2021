...
int main()
{
    ...
    if (fork() == 0)
    {
        ...
        {
            printf("Error: Child process read error!");
            exit(1);
        }
        // 打印读取到的字符数组
        ...
        {
            printf("Error: Child process write error!");
            exit(1);
        }
        exit(0);
    }
    ...
    {
        printf("Error: Parent process write error!");
        exit(1);
    }
    // 父进程从pipe2的读端，读取字符数组
    if (read(p2[0], buffer, length) != length)
    {
        printf("Error: Parent process read error!");
        exit(1);
    }
    // 打印读取的字符数组
